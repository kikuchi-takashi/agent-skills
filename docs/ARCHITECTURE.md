...
インストール先はコレクションルート外でなければならず、ソースと重なる配置を拒否します。複数スキルの導入では、同じファイルシステム上の一時領域へ全件コピーしてから出力先へ切り替えます。`--force` では既存版を一時退避し、CLIが処理できるコピーまたは切り替えエラーが起きた場合は、新規配置を除去して既存版を復元します。復元自体に失敗した場合は自動削除せず、エラーに復旧データの場所を示します。

コレクション一括インストールでも全出力先をコピー前と切り替え前に検査します。`--force` なしでは、どれか1つでも既存スキルと衝突すれば何も書き込みません。
