...
**高忠実度の描画**が使えるかを `shutil.which("soffice")` と `shutil.which("pdftoppm")` で確かめる。両方そろうか、ハーネス自身が PowerPoint を画像にする手段を持つなら、簡易描画で位置と構造を確認したあとに再描画し、字形・影・図表の見え方を見る。どちらも無ければ「探したが無かった」と記録し、簡易描画までを確認範囲として報告する。
