...
| `pptx-create` | 要件、原稿、資料から新しいデッキを生成する | `.pptx`、ブリーフ、構成、スライド実装仕様書、設計〜実装対応表、QA証跡 |
| `pptx-edit` | 既存デッキの内容やページを、元のデザインシステムに合わせて編集する | 編集済み `.pptx`、変更内容、必要時の実装仕様書と対応表、QA証跡 |
| `pptx-review` | デッキを変更せず、構造、レイアウト、文章、デザインの整合を監査する | 形式的/デザイン的監査報告、lint結果、個別・一覧描画、QA証跡 |
...
4. 構成とロックからスライド実装仕様書を作り、代表ページを個別・一覧の両方で事前previewする。
5. デザインロックと実装仕様から編集可能な `.pptx` を生成または編集する（`pptx-create` / `pptx-edit`）。
6. 生成後に設計〜実装対応表を作り、形式的QAとデザイン的QAを分けて全ページを検査する。
7. 成果物、仮定、要確認事項、検証結果をまとめて納品する。
...
- 全デッキで、生成前にスライド実装仕様書を作る。3ページ以下は全ページ、4ページ以上は最低3ページの高リスクな代表ページを、個別と一覧の両方で事前previewする。
- 生成後に設計〜実装対応表を作る。形式的QA（内容・ファイル・lint・仕様一致）とデザイン的QA（個別・一覧・署名・形式・主役・リズム）は独立判定し、全ページ固有の目視所見を残す。
...
品質確認は、形式的QAとデザイン的QAを混ぜず、次の証拠を積み上げます。

1. `pptx_lint.py` で全ページの構造、配置、文字、パッケージ整合を検査する。
2. `render_preview.py` で全ページ画像とコンタクトシートを生成し、全ページを1枚ずつ確認した固有所見と、一覧での全体所見を残す。
3. ハーネス自身がPPTXを画像にする手段を持つ場合だけ、最終成果物を見比べて表示差を確認する。
4. `qa_evidence.py` で形式的QA、デザイン的QA、設計〜実装対応、個別・一覧目視の完了を検査する。
