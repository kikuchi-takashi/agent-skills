...
| 選ばせる | `options-led` | 論点 → 選択肢 → 評価軸 → 比較 → 決めどころ | 合意形成、方式選定、投資判断 |
| 進捗を報告する | `plan-actual` | 予定 → 実績 → 差分 → 対策 | 定例報告、月次、プロジェクト会議 |
| 事故を報告する | `incident` | 事実 → 影響 → 原因 → 再発防止 | 障害報告、品質事故、監査対応 |
...
```json
"grammar": {"id": "options-led", "arc": ["論点", "選択肢", "評価軸", "比較", "決めどころ"],
            "escalate": ["EQUAL_EMPHASIS", "TEXT_DENSE"]}
```

- **原型の並び**: `statement`（論点） → `comparison` か `structure`（選択肢） → `table`（評価軸と比較） → `closing`（決めどころ）
- **やらないこと**: 推す案を隠さない（推奨は明示し、その上で選ばせる）。選択肢を4つ以上出さない
- **なぜその `escalate`**: 選択肢を等分・同色で並べると「どれでもいい」に見える。評価軸の表は詰め込みに転びやすい
- **`answer-led` との違い**: あちらは答えを出して承認を得る。こちらは**決めるのは相手**で、こちらは軸を用意する

## `plan-actual` — 進捗を報告する

```json
"grammar": {"id": "plan-actual", "arc": ["予定", "実績", "差分", "対策"],
            "escalate": ["LAYOUT_MONOTONE", "TITLE_TOPIC_LABEL"]}
```

- **原型の並び**: `roadmap`（予定） → `metrics` か `table`（実績） → `hero-number` か `trend`（差分） → `steps`（対策）
- **やらないこと**: 差分の段を飛ばさない（予定と実績を並べただけで終わらない）。良い数字だけを出さない
- **なぜその `escalate`**: 定例は毎回同じ形になり、読まれなくなる。「進捗報告」のような話題ラベルは差分を隠す
- **`chronicle` との違い**: あちらは経緯の物語。こちらは**約束との差**が主題

## `incident` — 事故を報告する

```json
"grammar": {"id": "incident", "arc": ["事実", "影響", "原因", "再発防止"],
            "escalate": ["AI_VOCAB", "PLACEHOLDER_TEXT", "DARK_PAGE_OVERUSE"]}
```

- **原型の並び**: `claim-evidence`（事実） → `hero-number` か `table`（影響） → `structure` か `steps`（原因） → `steps`（再発防止）
- **やらないこと**: 原因を人に帰さない（仕組みで書く）。影響を小さく見せない。**確定していないことを書かない**
- **なぜその `escalate`**: 誇張語彙は事故報告では不誠実に映る。未確定の仮置き文言が残ると信用を失う。濃い面の演出は場に合わない
- **`chronicle` との違い**: あちらは何が起きたかを語る。こちらは**なぜ起きたか**と**次はどう防ぐか**が主題
